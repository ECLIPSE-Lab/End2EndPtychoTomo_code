# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/solvers.ipynb.

# %% auto 0
__all__ = [ 'affine_matrix_3D_ZYX', 'rotate_volume', 'affine_matrix_2D_translate', 'translate_projections',
           'torch_ray_transform', 'ray_transform', 'set_projection_vectors', 
           'LinearTomographyInputs', 'LinearTomographyOptions', 'LinearTomographyResults', 'LinearTomographySolver',
           'TomographyAlignmentInputs', 'TomographyAlignmentOptions', 'TomographyAlignmentResults',
           'TomographyAlignmentSolver']
...
def rotate_volume(vol, phi_rad, theta_rad, psi_rad):
...
def affine_matrix_2D_translate(translations):
...
def translate_projections(projections, translations):
    n_theta, y, x = projections.shape
    projections = projections.unsqueeze(1)
    scale_factor = th.tensor([2 / y, 2 / x], dtype=th.float32, device=translations.device).reshape(2, 1)
...
def torch_ray_transform(vol, phi_rad, theta_rad, psi_rad, translations):
...
def ray_transform(vol, phi_rad, theta_rad, psi_rad, translations):
...
def set_projection_vectors(phi_rad, theta_rad, psi_rad, translation):
...
class LinearTomographyOptions:
    ...
    tau: float = 1e-4 # shrink non-negative values
    reg_pars: dict = field(default_factory=lambda: {
        'algorithm': 'FGP_TV',
    ...
    gauss_kernel: th.Tensor = None # Shape (N, N, N)
    edge_reg_pars: dict = field(default_factory=lambda: {
...
class LinearTomographySolver:
    def __init__(self, inputs: LinearTomographyInputs, reconstruction_options: LinearTomographyOptions):
        self._inputs = inputs
        self._options = reconstruction_options
            
        self.dataset = th.as_tensor(self._inputs.dataset, device=self._options.device, dtype=th.float32)
        self.volume = th.as_tensor(self._inputs.volume, device=self._options.device, dtype=th.float32)
        self._inputs.phi = th.as_tensor(self._inputs.phi, device=self._options.device, dtype=th.float32)
        self._inputs.theta = th.as_tensor(self._inputs.theta, device=self._options.device, dtype=th.float32)
        self._inputs.psi = th.as_tensor(self._inputs.psi, device=self._options.device, dtype=th.float32)
        self._inputs.translations = th.as_tensor(self._inputs.translations, device=self._options.device, dtype=th.float32)
        
        self.edge_penalty_tensor = self.init_edge_tensor(self.volume, self._options.edge_reg_pars)
    ...
    def solve(self):
        ...
        # Get options
        batch_size = self._options.batch_size if self._options.batch_size > 0 else sino_target.shape[0]
        ...
        sampler = BatchSampler(SequentialSampler(range(sino_target.shape[0])), batch_size=batch_size, drop_last=False)
        ...
        for epoch in epochs:
            ...
            with th.no_grad():
                ...
                if reg_pars is not None:
                    volume[:, :, :] = self.volume_regularization(volume, reg_pars, self._options.device)
                if gauss_kernel is not None:
                    volume[:, :, :] = filter3d(volume.detach().unsqueeze(0).unsqueeze(0), gauss_kernel.unsqueeze(0))
    ...
    @staticmethod
    def volume_regularization(volume, reg_pars, device):
        ...
        (vol, info_vec_gpu) = FGP_TV(vol,
                                     ...
                                     reg_pars['methodTV'],
                                     reg_pars['nonneg'], 'gpu')
        return th.as_tensor(vol, device=device)
    
    def init_edge_tensor(self, volume, edge_reg_pars):
    ...
    def edge_penalty(self, volume, edge_reg_pars):
        term = volume * self.edge_penalty_tensor
        num_elements = term.numel()
        reg_term = th.abs(term)
        return th.sum(reg_term) / num_elements * edge_reg_pars['regularisation_parameter']
...
class TomographyAlignmentOptions:
    ...
    num_iterations: int = 100
    optimizer: th.optim.Optimizer = th.optim.Adam
    to_fit: dict = field(default_factory=lambda: {
        'phi': True, 'theta': True, 'psi': True, 'translations': True})
    optimizers_params: dict = field(default_factory=lambda: {
        'phi':{'lr': 1e-4},
        'theta':{'lr': 1e-4},
        'psi':{'lr': 1e-5},
    ...
    # Regularization options
    angle_reg_options: dict = field(default_factory=lambda : {
...
class TomographyAlignmentSolver:
    def __init__(self, inputs: TomographyAlignmentInputs, reconstruction_options: TomographyAlignmentOptions):
        self._inputs = inputs
        self._options = copy(reconstruction_options)
        
        device = self._options.linearTomographyOptions.device    
        self.dataset = th.as_tensor(self._inputs.dataset, device=device, dtype=th.float32)
        self.volume = th.as_tensor(self._inputs.volume, device=device, dtype=th.float32)
        self._inputs.phi = th.as_tensor(self._inputs.phi, device=device, dtype=th.float32)
        self._inputs.theta = th.as_tensor(self._inputs.theta, device=device, dtype=th.float32)
        self._inputs.psi = th.as_tensor(self._inputs.psi, device=device, dtype=th.float32)
        self._inputs.translations = th.as_tensor(self._inputs.translations, device=device, dtype=th.float32)
    ...
    def solve(self):
        ...
        losses = []
        results = TomographyAlignmentResults(volume, phi, theta, psi, translations, losses)

        # Get options
        inner_options = self._options.linearTomographyOptions
        batch_size = inner_options.batch_size if inner_options.batch_size > 0 else sino_target.shape[0]
        num_iterations = self._options.num_iterations
        optimization_algorithm = self._options.optimizer
        ...
        inputs = LinearTomographyInputs(self._inputs.dataset, self._inputs.volume, phi, theta, psi, translations)
        inner_solver = LinearTomographySolver(inputs, inner_options)

        optimizers = [optimization_algorithm([phi], **(optimizers_parameters['phi'])),
                      optimization_algorithm([theta], **(optimizers_parameters['theta'])),
                      optimization_algorithm([psi], **(optimizers_parameters['psi'])),
                      optimization_algorithm([translations], **(optimizers_parameters['translations']))]
        ...
        for epoch in epochs:
            ...
            loss_angles = self.angles_regularization_diff(phi, theta, psi, angle_reg_pars, angle_reg_options)
    ...
    @staticmethod
    def angles_reg_init(angle_reg_pars, theta):
        ...
        angle_reg_pars = {'theta': theta_copy,
                          'k_phi': (width_phi ** 2),
    ...
    @staticmethod
    def angles_regularization_diff(phi, theta, psi, angle_reg_pars, angle_reg_options):
        if angle_reg_options['regularisation_parameter'] == 0:
            return th.tensor(0)

        reg_loss_phi = th.mean(th.square(th.diff(phi))) / angle_reg_pars['k_phi']
        reg_loss_theta = mse_loss(theta, angle_reg_pars['theta']) / angle_reg_pars['k_theta']
