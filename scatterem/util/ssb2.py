# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/util.ssb2.ipynb.

# %% auto 0
__all__ = ['PARAM_PREFIX', 're', 'im', 'cexp', 'cabs', 'cabs2', 'cmul', 'disk_overlap_kernel',
           'double_overlap_intensitities_in_range', 'find_rotation_angle_with_double_disk_overlap', 'focused_probe',
           'Param', 'fftshift_checkerboard', 'ZernikeProbeSingle', 'aperture', 'cartesian_aberrations',
           'cartesian_aberrations_single', 'single_sideband_reconstruction', 'single_sideband_kernel', 'sector_mask',
           'HSV_to_RGB', 'zplot', 'plotAbsAngle', 'plotcxmosaic', 'plotmosaic', 'plot', 'mosaic', 'P1A_to_HSV',
           'imsave', 'plot_complex_multi', 'plotcx', 'disk_overlap_function', 'wavelength', 'DOF', 'get_qx_qy_1D',
           'get_qx_qy_2D', 'get_qx_qy_1D_th', 'get_qx_qy_2D_th', 'center_of_mass_kernel',
           ...
           'sparse_to_dense_datacube', 'fftshift_kernel', 'fftshift_pad_kernel', 'virtual_annular_image_kernel',
           'crop_symmetric_around_center_kernel', 'crop_symmetric_around_center', 'rotate_kernel', 'rotate',
           'sum_kernel', 'sum_frames', 'rebin_kernel', 'array_split_divpoints_ntotal', 'dense_to_sparse_kernel',
           'advanced_raster_scan', 'rel_dist', 'R_factor', 'sgn', 'cangle', 'complex_numpy', 'cx_from_numpy',
           'make_real', 'make_imag', 'complex_polar', 'complex_expi', 'complex_exp', 'complex_mul', 'complex_mul_conj',
           'complex_mul_real', 'complex_div', 'iscomplex', 'check_complex', 'complex_matmul_conj', 'complex_matmul',
           'conj', 'ComplexMul', 'ComplexAbs', 'ComplexAbs2', 'ComplexExp']

# %% ../../nbs/util.ssb2.ipynb 2
import numpy as np
import time
...
def single_sideband_reconstruction(G, Qx_all, Qy_all, Kx_all, Ky_all, aberrations, theta_rot, alpha_rad,
    ...
    strides = cp.array((np.array(G.strides) / (G.nbytes / G.size)).astype(np.int32))
    scale = 1
...
def disk_overlap_function(Qx_all, Qy_all, Kx_all, Ky_all, aberrations, theta_rot, alpha, lam):
    ...
    threadsperblock = 2 ** 8
    blockspergrid = m.ceil(np.prod(gs) / threadsperblock)
    strides = cp.array((np.array(Gamma.strides) / (Gamma.nbytes / Gamma.size)).astype(np.int32))
    disk_overlap_kernel[blockspergrid, threadsperblock](Gamma, strides, Qx_all, Qy_all, Kx_all, Ky_all, aberrations,
                                                        theta_rot, alpha, lam)
    th.cuda.synchronize()
    return Gamma
...
def get_qx_qy_2D(M, dx, dtype, fft_shifted=False):
    ...
    if fft_shifted:
        qxn = th.fft.fftshift(qxn)
        qyn = th.fft.fftshift(qyn)
    return qxn, qyn
...
def crop_symmetric_around_center(old_frames, old_frame_dimensions, center_data, max_radius):
    ...
    threadsperblock = (16, 16)
    blockspergrid = tuple(np.ceil(np.array(old_frames.shape[:2]) / threadsperblock).astype(np.int32))
...
def rotate(frames, old_frame_dimensions, center, angle_rad):
    threadsperblock = (16, 16)
    blockspergrid = tuple(np.ceil(np.array(frames.shape[:2]) / threadsperblock).astype(np.int32))
